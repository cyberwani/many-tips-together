 ...
 */
  ...
  class BF_Admin_Page_Class {
    ...
    public function __construct($args) {
      ...
      add_action( 'wp_ajax_nopriv_apc_delete_mupload', array( $this, 'wp_ajax_delete_image' ) );
      //import export
    ...
    /**
      ...
      public function DisplayPage() {
        ...
        echo '<div class="wrap">';
        echo '<form method="post" action="" name="mttform" enctype="multipart/form-data">
  ...
  public function load_scripts_styles() {
    ...
    wp_enqueue_script( 'Admin_Page_Class', $plugin_path . '/js/Admin_Page_Class.js', array( 'jquery' ), time(), true );
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px;}
  ...
  public function show_field_checkbox_list( $field, $meta ) {
    ...
    $this->show_field_begin($field, $meta);
      ...
      foreach ($field['options'] as $key => $value) {
        $html[] = "<label class='no-class'><input type='checkbox' class='at-checkbox_list".(isset($field['class'])? " {$field['class']}": "")."' name='{$field['id']}[]' value='{$key}'" . checked( in_array( $key, $meta ), true, false ) . " />{$value}</label>";
      }
    ...
    if ('checkbox_list' == $options['type']) {
      foreach ($posts as $p) {
        if (isset($field['class']) && $field['class']== 'no-toggle')
          echo "<label class='no-class'><input type='checkbox' class='at-posts-checkbox".(isset($field['class'])? " {$field['class']}": "")."' name='{$field['id']}[]' value='$p->ID'" . checked(in_array($p->ID, $meta), true, false) . " /> {$p->post_title}</label>";
        else
  ...
  public function show_field_taxonomy($field, $meta) {
    ...
    if ('checkbox_list' == $options['type']) {
      foreach ($terms as $term) {
        if (isset($field['class']) && $field['class'] == 'no-toggle')
          echo "<label class='no-class'><input type='checkbox' class='at-tax-checkbox".(isset($field['class'])? " {$field['class']}": "")."' name='{$field['id']}[]' value='$term->slug'" . checked(in_array($term->slug, $meta), true, false) . " /> {$term->name}</label>";
        else
  ...
  public function show_field_WProle($field, $meta) {
    ...
    if ($names){
      ...
      if ('checkbox_list' == $options['type']) {
        foreach ($names as $n) {
          if (isset($field['class']) && $field['class'] == 'no-toggle')
            echo "<label class='no-class'><input type='checkbox'  class='at-role-checkbox".(isset($field['class'])? " {$field['class']}": "")."' name='{$field['id']}[]' value='$n'" . checked(in_array($n, $meta), true, false) . " /> ".$n."</label>";
          else
